    ...
    <div className="min-h-screen bg-white">
      <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 shadow-[0_4px_18px_rgba(15,23,42,0.05)] backdrop-blur-lg">
        <div className="mx-auto flex h-[4.5rem] max-w-7xl items-center gap-3 px-4 sm:px-6">
          ...
          <nav className="ml-auto hidden items-center gap-1 md:flex" aria-label="Role navigation">{links.map((item) => <Link key={item.href} to={item.href} className={`rounded-lg px-3 py-2 text-sm font-bold ${pathname.startsWith(item.href) ? 'bg-blue-50 text-[#003172]' : 'text-slate-600 hover:bg-slate-50'}`}>{item.label}</Link>)}</nav>
          <span className={`ml-auto flex items-center gap-1.5 whitespace-nowrap text-xs font-bold md:ml-2 ${syncStatus === 'offline' ? 'text-amber-800' : syncStatus === 'synced' ? 'text-emerald-700' : 'text-[#003172]'}`} title={status}>
            <StatusIcon className="h-4 w-4" />
            <span className="hidden sm:inline">{status}</span>
            <span className="sr-only sm:hidden">{status}</span>
          </span>
          <button type="button" onClick={() => setOpen((value) => !value)} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-blue-50 text-[#003172] transition-colors hover:bg-blue-100 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-200 md:hidden" aria-label="Toggle role menu" aria-expanded={open}>{open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button>
        </div>
