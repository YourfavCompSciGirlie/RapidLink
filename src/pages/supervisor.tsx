...
function availability(state: Parameters<typeof employeeDuty>[0], employeeId: string) {
  ...
  return {
    onDuty,
    busy,
    label: !onDuty ? 'Off duty' : busy ? 'Busy' : 'Available',
    classes: !onDuty ? 'bg-slate-100 text-slate-700' : busy ? 'bg-amber-100 text-amber-900' : 'bg-emerald-100 text-emerald-900',
  };
}
...
export default function SupervisorPage() {
  ...
  const onDutyCount = stationEmployees.filter((employee) => employeeDuty(state, employee.id)).length;
  const availableCount = stationEmployees.filter((employee) => employeeDuty(state, employee.id) && !employeeBusy(state, employee.id)).length;
      ...
      <div className="mx-auto max-w-6xl px-4 py-5 sm:px-6 sm:py-8">
        <header className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_14px_38px_rgba(15,23,42,0.08)]">
          ...
          <div className="grid grid-cols-3 divide-x divide-slate-200 border-y border-slate-100 bg-slate-50/80 py-3 text-center">
            <div className="px-2"><p className="text-xl font-black text-[#003172]">{stationEmployees.length}</p><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Employees</p></div>
        ...
        <section className="mt-6" aria-labelledby="team-heading">
          <div className="flex items-end justify-between gap-3">
            <div><h2 id="team-heading" className="text-lg font-bold text-[#003172]">Employees</h2><p className="mt-0.5 text-xs font-semibold text-slate-500">{employees.length} shown</p></div>
          </div>

          <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_190px]">
            <label className="relative"><span className="sr-only">Search employees</span><Search className="absolute left-3 top-3.5 h-5 w-5 text-slate-400" /><input className="h-12 w-full rounded-xl border border-slate-200 bg-white pl-11 pr-3 text-base outline-none transition focus:border-[#003172] focus:ring-4 focus:ring-blue-100" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search employees" /></label>
              ...
              return (
                <article key={employee.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.06)]">
                  ...
                  <dl className="mt-4 grid grid-cols-2 gap-3 rounded-xl bg-slate-50 p-3 text-xs">
                    <div><dt className="font-semibold text-slate-500">Phone</dt><dd className="mt-0.5 font-bold text-slate-900">{employee.phone}</dd></div>
                    <div><dt className="font-semibold text-slate-500">Record</dt><dd className="mt-0.5 font-bold text-slate-900">{employee.active ? 'Active' : 'Inactive'}</dd></div>
                  </dl>
                  <Button variant="outline" className="mt-3 min-h-11 w-full" onClick={() => setEditing(employee)}><Pencil className="h-4 w-4" /> Edit employee</Button>
                </article>
              );
            })}
          </div>
          ...
          <div className="mt-4 hidden overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_8px_24px_rgba(15,23,42,0.06)] md:block">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {employees.map((employee) => {
                  const status = availability(state, employee.id);
                  return <tr key={employee.id} className="align-middle"><td className="px-4 py-4"><p className="font-extrabold text-slate-950">{employee.name} {employee.surname}</p><p className="mt-0.5 text-xs font-semibold text-slate-500">{employee.employeeNumber}</p></td><td className="px-4 py-4 font-bold text-slate-700">{serviceLabel(employee.service)}</td><td className="px-4 py-4 font-semibold text-slate-700">{employee.phone}</td><td className="px-4 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${status.classes}`}>{status.label}</span></td><td className="px-4 py-4 font-bold text-slate-700">{employee.active ? 'Active' : 'Inactive'}</td><td className="px-4 py-4 text-right"><Button size="sm" variant="outline" onClick={() => setEditing(employee)}><Pencil className="h-4 w-4" /> Edit</Button></td></tr>;
                })}
              </tbody>
            </table>
          </div>

          {!employees.length && <div className="mt-4 rounded-2xl border border-dashed border-slate-300 bg-white px-5 py-10 text-center"><span className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-slate-100 text-slate-500"><UserX className="h-6 w-6" /></span><p className="mt-3 font-extrabold text-slate-950">No matching employees</p><p className="mt-1 text-sm text-slate-500">Try another name or service.</p></div>}
        </section>
      </div>

      <AccessibleModal open={editing !== null} title={editing === 'new' ? 'Add employee' : 'Edit employee'} description="Attendance is managed separately." onClose={() => setEditing(null)}>
